...
(progn
 ...
 (defq
  ...
  (mlambda
   (x  .  y )
   (list 'if (list 'not x) (cons 'progn y))))
 (defq
  eek
  (nlambda
   (THISNAME)
  ...
  (lambda
   ...
   (while
    ...
    (cond
     ...
     ((eq ch 'n) (setq x (eeinsert v x)) (eeprint x))
     ((eq ch 'y)
      (push THIS JEMMA)
      (if
       (eqn v 0)
       ...
       ((function
         (lambda
          ...
          (while
           ...
           (if
            (atom x)
            (progn (printc 46) (sp) (print x) (setq x nil))
            (if
             (lessp dec 0)
             (progn (print '&))
             (if
              (atom (car x))
              (progn (print (car x)) (if (cdr x) (sp)))
              (if
               (lessp 1 (depthless dec (car x)))
               (progn (print (car x)))
               (if t (progn (print '&) (sp)) ())))))
           (setq dec (plus dec -3))
  ...
  (lambda
   ...
   ((function
     (lambda
      (p)
      (repeat-times
       (quotient (difference (plus 22 1) 0) 1)
       (if
        (nthcdr p x)
        (progn (tab 3) (eeprint25 (nth p x)) (cr)))
       (setq p (plus p 1)))))
    0)
 ...
 (defq
  BOOTPY
  (compile mapp save-module unless eek eeinsert locate koe depthless nedit eeprint25 eeprint fib save BOOTPY)))
